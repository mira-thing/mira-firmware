...
cp -a "$SCRIPTS_PATH"/services/mira-ui "$ROOTFS_PATH"/etc/sv/

mkdir -p "$ROOTFS_PATH"/etc/mira/ui
unzip "$UI_ZIP" -d "$ROOTFS_PATH"/etc/mira/ui

DEFAULT_SERVICES="${DEFAULT_SERVICES} mira-ui"
